...
@section('content')
    ...
    <div class="row" id="app">
        <div class="panel panel-info">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5 col-lg-3">
                        ...
                        @foreach($tipos_ambiente as $tipo_ambiente)
                            <option value="{{ $tipo_ambiente->id }}" {{ ($ambiente->tipo_id == $tipo_ambiente->id) ? 'selected' : '' }}>{{ $tipo_ambiente->nombre }}</option>
                        @endforeach
                        </select>
                    </div>
                </div>
            ...
            <input type="hidden" name="facultad_x_sede_id" value="{{ $ambiente->facultad_x_sede_id }}">
            </form>
        </div>
    </div>
